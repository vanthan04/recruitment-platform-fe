...
export function JobRow({ job }: { job: Job }) {
  return (
    <div className="border-border hover:border-primary flex items-center justify-between gap-3 rounded-xl border p-4 shadow-sm transition-colors">
      <div className="min-w-0">
        <Link href={PATH.RECRUITER_JOB_APPLICATIONS(job.id)} className="font-medium hover:underline">
          {job.title}
        </Link>
        <div className="mt-1.5 flex flex-wrap items-center gap-2">
          <Badge variant={STATUS_VARIANT[job.status]} className="rounded-full">
